// MultiTenant.Enforcer.EntityFramework/TenantDbContext.cs
using System;
using System.Linq;
...
namespace MultiTenant.Enforcer.EntityFramework
{
    ...
    public abstract class TenantDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // Configure tenant isolation for all ITenantIsolated entities
            ConfigureTenantIsolation(modelBuilder);
        }

        public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
        {
            // Set tenant IDs on new entities and validate tenant ownership
        ...
        {
            // Force async usage for better performance and consistency
        ...
        private void ProcessTenantIsolatedEntities()
        {
            ...
            foreach (var entry in addedEntities)
            {
                ...
                {
                    // System context requires explicit tenant ID
        ...
        /// <summary>
        /// Creates a new instance of the specified tenant repository.
        /// </summary>
        /// <typeparam name="T">The entity type</typeparam>
        /// <returns>A tenant repository instance</returns>
        public ITenantRepository<T> GetTenantRepository<T>() where T : class, ITenantIsolated
        {
            // This would typically be resolved from DI container
            // For now, create directly (you'll want to inject IServiceProvider for proper DI)
            throw new NotImplementedException(
                "Use dependency injection to resolve ITenantRepository<T> instead of this method");
        }

        /// <summary>
        /// Disables tenant filtering temporarily for advanced scenarios.
        /// USE WITH EXTREME CAUTION - Only for system operations.
        /// </summary>
        /// <returns>Disposable that restores filtering when disposed</returns>
        protected IDisposable DisableTenantFiltering()
        {
            // This would temporarily disable global query filters
            // Implementation depends on your specific needs
            _logger.LogWarning("Tenant filtering disabled - ensure this is authorized");
            
            return new TenantFilteringDisabler(this, _logger);
        }

        private class TenantFilteringDisabler : IDisposable
        {
            private readonly TenantDbContext _context;
            private readonly ILogger _logger;
            private bool _disposed;

            public TenantFilteringDisabler(TenantDbContext context, ILogger logger)
            {
                _context = context;
                _logger = logger;
                
                // In EF Core 8, you might use IgnoreQueryFilters() on specific queries
                // rather than globally disabling filters
            }
            ...
            public void Dispose()
            {
                if (!_disposed)
                {
                    _logger.LogWarning("Tenant filtering re-enabled");
                    _disposed = true;
                }
            }
        }
